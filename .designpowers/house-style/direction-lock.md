...
Palette strategy: mono+1 with semantic status colors and bounded MTA interaction accents
Palette values: canonical custom properties in `web/src/styles.css`, including `--paper`, `--surface`, `--panel`, `--text`, `--text-soft`, `--accent`, `--go`, `--warn`, `--stop`, and theme-adapted `--mta-*-ui` interaction tokens
...
Depth: borders only
Motion budget: animates opacity, supported disclosure transitions, and button feedback; never animates layout-critical position or required meaning; duration 200ms maximum; all new motion is removed under `prefers-reduced-motion`
